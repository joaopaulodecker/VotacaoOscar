...
class ControladorCadastro:
    ...
    def cadastrar(self):
        ...
        if dados:
            if "id" in dados and self.existe_id(dados["id"]):
                print(f"❌ Já existe um(a) {self.__tipo_entidade} com o ID {dados['id']}.")
            else:
                self.__entidades.append(dados)
                print(f"✅ {self.__tipo_entidade.capitalize()} cadastrado(a) com sucesso!")
        else:
    ...
    def alterar(self):
        ...
        if entidade_encontrada:
            ...
            if novos_dados:
                novo_id = novos_dados.get("id")
                if novo_id != id_alvo and self.existe_id(novo_id):
                    print(f"❌ Já existe outro(a) {self.__tipo_entidade} com o novo ID {novo_id}.")
                else:
